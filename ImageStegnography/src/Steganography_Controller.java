			...
			if( ( jpf.getText().equals(jpf1.getText()) ) && (jpf.getText().length()>=8 && jpf.getText().length()<=16) )	{
				if (returnVal == JFileChooser.APPROVE_OPTION){
					...
					try{
						...
						String orig = jpf.getText();
						Steganography_Controller.validate(orig);
						String text = SimpleProtector.encrypt(text1);
				...
				JOptionPane.showMessageDialog(view,"Passwords did not match (OR) Password legth should be (8-16)");
			}
		}
